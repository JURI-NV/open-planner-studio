...
- **AI-backup:** staat de toggle aan, dan schrijft de tool-laag bij de **eerste muterende tool-aanroep per document** (per server-sessie; teller reset na een handmatige backup) eerst een IFC-snapshot naar `<appDataDir>/ai-backups/<projectnaam>-<timestamp>.ifc` — zelfde `ifcWriter`-route als opslaan/recovery. Opruimbeleid: laatste 10 per document. De backup verschijnt als regel in het activiteitenpaneel en wordt vermeld in de envelop van die eerste mutatie; mislukt de backup, dan wordt de mutatie **geweigerd** met een duidelijke fout (fail-safe, geen stil doorgaan). "Backup-map openen" gebruikt de bestaande shell-open-plugin; terugzetten = het bestand gewoon openen als project.
