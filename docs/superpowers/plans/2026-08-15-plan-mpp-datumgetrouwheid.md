...
**H1/H2 — de 6 bestanden die zonder melding afwijken (eindreview T16c, gemeten 2026-08-17).** De gids beweerde tot deze fixronde "hoogstens één taak in een ongebruikelijke situatie" — een claim die nooit tegen het volledige corpus gemeten was en de bekende (c)-dossiers/detectiegrenzen negeerde. Corpusbrede meting (`sourceScheduleNotes.total`, de teller achter de leveling/split/resource-gedreven-melding, per bestand tegen de fidelity-afwijkingen): van de bestanden met ≥1 start-/finish-afwijking geven er **6 GEEN enkele melding**, verspreid over drie categorieën:
...
| één bedrijfsbestand (hash-gesleuteld, geen naam — §6-privacybeleid) | 1 taak, `sameday` (45 min, niet in de `startDiff`-emmer omdat de dag klopt) | dossier (c)5 zelf — het "1 RAW"-geval uit de SNET/MSO-probe; dit specifieke bestand toont óók een leveling-melding (`sourceScheduleNotes.total: 15`, voor 15 ANDERE taken) — de (c)5-taak zelf hangt niet samen met die nivellering en blijft dus alsnog ongemeld, TWEE onafhankelijke oorzaken in één bestand |
