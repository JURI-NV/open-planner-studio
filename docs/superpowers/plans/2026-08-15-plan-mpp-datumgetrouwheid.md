...
**Ontwerp.** Een werkende uitzondering = `IFCWORKTIME(name, .PREDICTED., $, #recurrence-met-IFCTIMEPERIODs, start, finish)` in dezelfde `ExceptionTimes`-lijst; een feestdag houdt `$` op arg 3. Backwards compatible: bestaande bestanden schrijven/lezen byte-identiek.
