...
**Attributie-tussenstand (T3-Opus-review, geïsoleerd gemeten):** alleen *W14 After Para 26* is T3 — en dáár is T3's expansie aantoonbaar correct (MSP's eigen opgeslagen datums bewijzen dat New Year/Christmas/Boxing Day meegerekend worden); de verslechtering is **ontmaskering** van een pre-existente fout uit een andere baan ("Delivery Plan" ligt 2 werkdagen mis zónder feestdag in het venster — kandidaat voor T15-classificatie). De overige 4 zijn níét T3; kandidaten T6/T11 (beide raken de OzBuild-set) — T13 meet dit na integratie van de T6-fixronde opnieuw en attribueert het restant. De delta-samenvatting bestaat inmiddels (T1-minironde); T13 verbetert alleen de formulering: "verslechterd" telt op de sóm start+finish, waardoor een gemengd bestand (−7 start/+3 finish) als "verbeterd" telt — schrijf "verslechterd (op de som)" of tel per veld, zodat niemand er "slechts N bestanden met nieuwe foute datums" uit afleidt.
