...
- **`moveProject.ts` wordt triviaal**: verdict `'keep'` (er staan geen datums in), aantoonbaar correct in plaats van een handgeschreven shift.
...
- [ ] `src/engine/moveProject.ts` — verdicts invullen. `splitGaps` = `'keep'` (offsets, geen datums — dat is het ontwerpvoordeel uit Z0); `manuallyScheduled` = `'keep'`; de manual-datums zitten in `scheduleStart`/`scheduleFinish` en dragen dus het bestaande verdict.
