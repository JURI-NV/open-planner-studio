...
**SAP PPM: nee, niet in dezelfde klasse — maar minder kaal dan aanvankelijk gesteld.** PPM heeft relatietypen (FS/SS/FF/SF), voor-/achterwaartse berekening, **een float-begrip én een kritiek pad met instelbare bepalingslogica** (zie de correctie in §2.4). Wat het niet heeft: een kleinere tijdseenheid dan **de dag**, P6-achtige float-rapportage en resource-nivellering op netwerkniveau. Het blijft daarmee primair een *governance*-planner, geen full-featured scheduler — maar de eerdere formulering "zonder float, zonder kritiek pad" was aantoonbaar onjuist.
...
- ~~Geen total/free float-uitvoer en geen kritiek-pad-markering.~~ **GECORRIGEERD (verificatie 25-07-2026):** PPM kent wél een float-begrip én een kritiek pad. SAP levert zelfs een instelbare *Alternative Calculation for Critical Path*: *"With this feature, you can change the logic used to determine the critical path of a project… When the New Logic for Critical Path Determination ('X'=Yes, '' = No) is enabled, tasks with a float of less than or equal to zero are flagged as part of the critical path only if their delays jeopardize the adherence to the project dates."* Component PPM-PRO, beschikbaar vanaf **PPM 6.1 SP14 / PPM 6.0 SP18**, ook uitgeleverd in S/4HANA 1709/1809/1909 ([SAP Help – Alternative Calculation for Critical Path](https://help.sap.com/docs/SAP_PORTFOLIO_AND_PROJECT_MANAGEMENT/47f3c81b7cc040f1aa74c2353651b6ac/898f5abb818d4a599646dd293018947a.html)). Wat blijft staan: PPM levert géén P6-achtige float-kolommen/­rapportage en geen resource-nivellering op netwerkniveau, en de dagresolutie (hieronder) beperkt de bruikbaarheid — maar "geen kritiek pad" was onjuist;
...
**Gantt in PPM.** *(Deze alinea is bij verificatie op 25-07-2026 op twee punten gecorrigeerd — zie §Verificatie.)*

De grafische view draaide jarenlang op een **Java-plug-in-applet**. Omdat de Java-plug-in uit moderne JDK's is verdwenen, bouwde SAP een **SAPUI5-Gantt** (PPM 6.1 SP13). In de bijbehorende what's-new somt SAP op wat daarin niet werkt of anders werkt. **Nog steeds geldig** in de actuele documentatie (PPM 6.1 **SP24**, bijgewerkt 14-05-2025): *"The Navigation function is not supported"*, *"Moving rows up or down within the hierarchy using drag and drop is not supported"*, *"Other view options and related settings are not supported"* (view alleen Jaar/Kwartaal/Maand/Kalenderweek via een zoomslider), en *"You have to confirm any change to a value in an editable field by choosing Enter before you can perform the next action using a Web Dynpro button."*

**Twee eerdere beweringen zijn onjuist gebleken:**
- **Printen is wél ondersteund.** SAP leverde *"Printing in Gantt Chart Based on SAPUI5 Technology… provides an option to print a Gantt chart visualization by generating a file in PDF format"*, **beschikbaar vanaf PPM 6.1 (SP14)** ([SAP Help](https://help.sap.com/docs/SAP_PORTFOLIO_AND_PROJECT_MANAGEMENT/47f3c81b7cc040f1aa74c2353651b6ac/fa588556dcd048a5a87600940894ab3e.html)). Het "Print function is not supported" geldt nog wél voor de **multi-project monitor**, niet voor de Gantt zelf.
- **De multi-project monitor ís vervangen.** Er bestaat een *"Alternative Multi-Project Monitor Based on SAPUI5 Technology"*, die in de actuele documentatie de standaard is: *"This SAPUI5-based multi-project monitor has replaced the former multi-project monitor based on Java Plug-In technology as the standard"* ([SAP Help – Multi-Project Monitor](https://help.sap.com/docs/SAP_PORTFOLIO_AND_PROJECT_MANAGEMENT/213a68a4ef6e448e8e49d8f67d1a1607/48b8e1b3021910b5e10000000a421138.html)).

Bovendien relativeert SAP's eigen actuele tekst de zwaarte van de regressie: *"The SAPUI5-based Gantt chart offers largely the same functional scope and user experience as the former Gantt chart based on Java Plug-In technology"*, en de SAPUI5-variant *"has replaced the former Gantt chart based on Java Plug-In as the standard Gantt chart"* ([SAP Help – Graphical View, PPM 6.1 SP24](https://help.sap.com/docs/SAP_PORTFOLIO_AND_PROJECT_MANAGEMENT/213a68a4ef6e448e8e49d8f67d1a1607/b185db48420643a5acef3ef3a3565f4a.html); [Alternative Gantt Chart based on SAPUI5 Technology](https://help.sap.com/docs/SAP_PORTFOLIO_AND_PROJECT_MANAGEMENT/47f3c81b7cc040f1aa74c2353651b6ac/1dee363b6e874dbdaaead050a56be96a.html)). De klacht "zwakke planningsergonomie" blijft onderbouwd door de resterende beperkingen (geen navigatie, geen drag-and-drop-herordening, vaste zoomniveaus, Enter-bevestiging), maar is **minder scherp** dan het oorspronkelijke profiel suggereerde.
...
**Belangrijke juridisch-technische waarschuwing — met een nuance die bij verificatie boven water kwam.** *"Graphics in Project System are part of the SAP S/4HANA compatibility scope, which comes with limited usage rights"*, met vervaldatum en details in SAP Note 2269324 (compatibiliteitsmatrix **ID 465**). **Maar** §29.20 van de Simplification List somt onder *"graphics that are only available in SAP GUI"* uitsluitend de **hiërarchiegrafiek** op (Note 3000979), en zegt over de andere twee expliciet het tegenovergestelde van "aflopend": *"Project Planning Board — SAP GUI-based project planning board **will continue to be available**"* en *"Network Graphic — SAP GUI-based network graphic **will continue to be available**"*, met de Fiori-apps *Project Schedule* (S/4HANA 2021), *Project Schedule for Versions* (2021 FPS1) en *Project Network Graph* (2020 FPS1) als **aanvullende** ("In addition") alternatieven. De eerdere formulering dat planbord én netwerkgrafiek "op geleende tijd leven" is dus te sterk. Voor de simulatiefuncties gold de compatibility scope aanvankelijk ook (matrix **ID 469**); die zijn per S/4HANA 2020 FPS2 / 2021 FPS1 wél naar de *perpetual scope* gehaald ([S/4HANA 2021 Simplification List, §29.20 en §29.21, p. 654–656](https://help.sap.com/doc/f2591a6901344c97a5e2029cc8f3703e/2021/en-US/SIMPL_OP2021.pdf); [SAP Help – Compatibility Scope](https://help.sap.com/docs/SAP_S4HANA_ON-PREMISE/c9b5e9de6e674fb99fff88d72c352291/9f17487cb7654c08baec66cfaa8867f7.html)). *De exacte einddatum staat in SAP Note 2269324, die alleen met SAP-login leesbaar is — die datum is dus **niet geverifieerd** in dit onderzoek en moet door de klant zelf worden nagegaan.*
...
1. **De planningsergonomie is zwak — SAP documenteert het zelf.** *(Bij verificatie afgezwakt; twee deelclaims bleken onjuist — zie §2.4 en §Verificatie.)* De PPM-Gantt moest van Java-applet naar SAPUI5 en SAP somt in de eigen documentatie op wat daarbij sneuvelde. **Nog steeds geldig in PPM 6.1 SP24 (2025):** *"The Navigation function is not supported"*, rijen verslepen in de hiërarchie niet ondersteund, view/zoom beperkt tot vier vaste niveaus via een slider (*"Other view options and related settings are not supported"*), en elke veldwijziging moet met Enter bevestigd worden voor de volgende actie. **Niet meer geldig:** printen is sinds PPM 6.1 SP14 wél ondersteund (PDF-export), en de multi-project monitor ís vervangen door een SAPUI5-variant die nu de standaard is. SAP stelt bovendien zelf dat de SAPUI5-Gantt *"offers largely the same functional scope and user experience"* als de Java-versie ([SAP Help – What's New](https://help.sap.com/docs/SAP_PORTFOLIO_AND_PROJECT_MANAGEMENT/47f3c81b7cc040f1aa74c2353651b6ac/1dee363b6e874dbdaaead050a56be96a.html); [Graphical View](https://help.sap.com/docs/SAP_PORTFOLIO_AND_PROJECT_MANAGEMENT/213a68a4ef6e448e8e49d8f67d1a1607/b185db48420643a5acef3ef3a3565f4a.html)). Een planner die uit P6 of MSP komt, ervaart het geheel nog steeds als een stap terug — maar minder dramatisch dan hierboven oorspronkelijk gesteld.
...
3. **De klassieke PS-grafiek zit in de compatibility scope — maar minder acuut dan gedacht.** *(Genuanceerd na verificatie.)* "Graphics in Project System" vallen onder de **compatibility scope met beperkte gebruiksrechten en een vervaldatum** (SAP Note 2269324, matrix-ID 465). SAP noemt in §29.20 echter alleen de **hiërarchiegrafiek** als "only available in SAP GUI", en stelt expliciet dat het **SAP GUI-planbord en de netwerkgrafiek *"will continue to be available"***, met de Fiori-apps als aanvulling. De vervaldatum staat in de niet-publieke Note 2269324 en is dus **niet geverifieerd**. Dat de Fiori-apps (*Project Schedule*, *Project Network Graph*) "read-/analyse-georiënteerd" zijn en minder bewerkingsdiepte bieden, is een **inschatting van de analist** — daar is in de SAP-documentatie geen expliciete uitspraak over gevonden ([Simplification List §29.20, p. 654–655](https://help.sap.com/doc/f2591a6901344c97a5e2029cc8f3703e/2021/en-US/SIMPL_OP2021.pdf)).
...
- **Planners en schedulers als primaire gebruikersgroep.** De ergonomie — trage GUI-planborden in compatibility scope, een UI5-Gantt zonder navigatiefunctie en zonder drag-and-drop-herordening, dagresolutie in PPM — jaagt hen weg. Zij blijven P6 of Asta gebruiken.
...
**Voor SAP PPM: nee.** Dat is een portfolio- en governancetool met een lichte planningslaag, geen CPM-tool. Wie het als vervanger van P6 of MSP positioneert, verkoopt iets anders dan hij levert. *(Wel corrigeren: PPM heeft aantoonbaar een float-begrip en een kritiek pad met instelbare bepalingslogica — zie §2.4. Het echte bezwaar is de dagresolutie en het ontbreken van P6-achtige float-rapportage en nivellering, niet het ontbreken van CPM-basisbegrippen.)*
...
*Toegevoegd tijdens de fact-check van 25-07-2026:*

45a. [Alternative Calculation for Critical Path (PPM 6.1 SP14 / 6.0 SP18) — bewijs dat PPM wél float én kritiek pad kent](https://help.sap.com/docs/SAP_PORTFOLIO_AND_PROJECT_MANAGEMENT/47f3c81b7cc040f1aa74c2353651b6ac/898f5abb818d4a599646dd293018947a.html)
45b. [Printing in Gantt Chart Based on SAPUI5 Technology (PPM 6.1 SP14) — weerlegt "printen niet ondersteund"](https://help.sap.com/docs/SAP_PORTFOLIO_AND_PROJECT_MANAGEMENT/47f3c81b7cc040f1aa74c2353651b6ac/fa588556dcd048a5a87600940894ab3e.html)
45c. [Multi-Project Monitor (SAPUI5, PPM 6.1 SP24) — weerlegt "multi-project monitor niet vervangen"](https://help.sap.com/docs/SAP_PORTFOLIO_AND_PROJECT_MANAGEMENT/213a68a4ef6e448e8e49d8f67d1a1607/48b8e1b3021910b5e10000000a421138.html)
45d. [Graphical View (PPM 6.1 SP24) — actuele "changed or not supported"-lijst van de SAPUI5-Gantt](https://help.sap.com/docs/SAP_PORTFOLIO_AND_PROJECT_MANAGEMENT/213a68a4ef6e448e8e49d8f67d1a1607/b185db48420643a5acef3ef3a3565f4a.html)
45e. [Factors Influencing Scheduling — S/4HANA 2025 FPS01-versie (bevestigt dat de minimum-only lag nog actueel is)](https://help.sap.com/docs/SAP_S4HANA_ON-PREMISE/4dd8cb7b1c484b4b93af84d00f60fdb8/0f73b65334e6b54ce10000000a174cb4.html)
...
**Adversariële fact-check, uitgevoerd 25 juli 2026.** Opzet: elke belangrijke falsifieerbare bewering actief proberen te *weerleggen* met onafhankelijke toegang tot de primaire bron. De drie G-Cloud-prijs-PDF's zijn deze keer **volledig uitgelezen** (pypdf, niet via een samenvattende proxy) en de Simplification List is op **paginaniveau** gecontroleerd. SAP Help-pagina's zijn JS-gerenderd en dus niet direct leesbaar; die zijn geverifieerd via de **SAP Help elasticsearch content-API** (`help.sap.com/http.svc/elasticsearch`), die de letterlijke zinnen uit de actuele documentatie teruggeeft inclusief product-, versie- en datumstempel.

### Bevestigd

| # | Bewering | Oordeel | Bron |
|---|---|---|---|
| 1 | EPPM-lijstprijzen Professional **£146 / £83 / £66** en Standard **£39,10 / £24,15 / £21,85**, **minimum 1 active user** voor beide, excl. btw, indexatie *"by the lower of CPI and 3.3%"*, *"Solution support is not included"*, doc v1.0 d.d. 1 mei 2024 | **bevestigd** — alle cijfers letterlijk uit de PDF (p. 3–4), geen enkele afwijking | [PDF](https://assets.applytosupply.digitalmarketplace.service.gov.uk/g-cloud-14/documents/702210/184802193487061-pricing-document-2024-05-01-1904.pdf) |
| 2 | Publieke listingsamenvatting **"£22 to £147 a user a month"** | **bevestigd** letterlijk (afronding van £21,85 en £146) | [G-Cloud-listing](https://www.applytosupply.digitalmarketplace.service.gov.uk/g-cloud/services/184802193487061) |
| 3 | RISE PE: Base **£485 vlak, min. 60 FUE**; Premium **£705→£29**, min. 60; Premium Plus **£975→£37**, min. 60; Premium Upgrade £272→£8,90 | **bevestigd** — alle negen staffels per editie exact gecontroleerd (p. 3–6) | [PDF](https://assets.applytosupply.digitalmarketplace.service.gov.uk/g-cloud-14/documents/702210/735410599987909-pricing-document-2024-05-02-1724.pdf) |
| 4 | Digital Access **£8,41 → £1,08**; extra productieve tenant **£4.186/mnd**; niet-productieve **£3.448/mnd**; Customer Evaluation System **£41.136 / £21.649** | **bevestigd** (p. 9, 13, 15, 17). *Nuance:* de Digital-Access-staffel is niet monotoon (10.001–15.000 = £4,76 ligt boven 5.001–10.000 = £4,14); de in het profiel genoemde eindpunten kloppen wel | idem |
| 5 | GROW Public Edition: Base **min. 15 FUE**, £487→£43; Premium **min. 25 FUE**, £496→£41; sandbox **£6.540/mnd** | **bevestigd** exact (p. 3–5) | [PDF](https://assets.applytosupply.digitalmarketplace.service.gov.uk/g-cloud-14/documents/702210/143142176115221-pricing-document-2024-05-01-1834.pdf) |
| 6 | Vendr: mediaan **$29.293/jaar** (range $11.660–$347.035), **55 deals**, ~24% korting, **$150–$400+ p/u/mnd**, **100–500 gebruikers → $500K–$3M+ ACV**, **17–22%** onderhoud, 15–25% meerjarenkorting | **bevestigd** — inclusief de letterlijke zin *"buyers with 100–500 users typically achieve total annual contract values ranging from $500K to $3M+"* | [Vendr](https://www.vendr.com/buyer-guides/sap) |
| 7 | Implementatiekosten *"can equal or exceed software subscription fees"* | **bevestigd** als letterlijk citaat | [Vendr](https://www.vendr.com/buyer-guides/sap) |
| 8 | Geen gratis tier, geen proefversie, geen publieke prijs op sap.com; alles via offerte/partner | **bevestigd** — geen zelfbedieningsaankoop of trial voor PS/PPM/EPPM aangetroffen; de enige harde lijstprijzen komen van een partner-raamwerk (G-Cloud), niet van SAP zelf | zie 1–5 |
| 9 | SAP PS-lag is **minimum-only**: *"Scheduling interprets the time interval of the relationships as a minimum time interval."* | **bevestigd** letterlijk, en **nog steeds actueel**: de zin staat identiek in de S/4HANA **2025 FPS01**-versie van *Factors Influencing Scheduling* | [SAP Help (S/4HANA 2025)](https://help.sap.com/docs/SAP_S4HANA_ON-PREMISE/4dd8cb7b1c484b4b93af84d00f60fdb8/0f73b65334e6b54ce10000000a174cb4.html) |
| 10 | PS kritiek pad = *"the path through the network in which the activities and their relationships are ordered so that the total float is minimum"*, *"in general the longest way"*, *"Activities with the smallest total float are called 'critical'"* | **bevestigd** letterlijk (alle drie de zinnen) | [SAP Help – Floats](https://help.sap.com/docs/SAP_ERP/5ecdd9085d344e6693e65fc60c3b5b0f/1e73b65334e6b54ce10000000a174cb4.html) |
| 11 | PPM plant in hele dagen: *"The time unit for the duration of project elements is Day."* | **bevestigd** letterlijk, in de **S/4HANA 2025 FPS01**-versie van de PPM-Scheduling-pagina | [SAP Help](https://help.sap.com/docs/SAP_S4HANA_ON-PREMISE/db719753e69f4e8eb9902aaea0fd8471/493246dde87f62b6e10000000a421138.html) |
| 12 | PPM: top-down niet mogelijk bij summary tasks; kalender per relatie | **bevestigd** (*"You cannot choose top-down scheduling if your project contains summary tasks"*; *"You can assign a calendar that is to be used when scheduling the time intervals of the relationship"*) | idem + [Maintaining Relationships](https://help.sap.com/docs/SAP_S4HANA_ON-PREMISE/db719753e69f4e8eb9902aaea0fd8471/497eede2d6c33d8be10000000a421138.html) |
| 13 | Simplification List §29.6 (p. 637–638): OpenPS4MSP, MPX incl. CN06/CN07, Palm-download, Graneda/MS Access **verwijderd**; advies EPC of PS-BAPI's | **bevestigd** op exact de genoemde pagina's | [SIMPL_OP2021.pdf](https://help.sap.com/doc/f2591a6901344c97a5e2029cc8f3703e/2021/en-US/SIMPL_OP2021.pdf) |
| 14 | Simplification List §29.12 (p. 645–646): *"Project System business functions for Handling Large Projects… are not the target architecture anymore in SAP S/4HANA"*, `OPS_PS_HLP_1/2` obsolete, PSHLP10/20/30/90 onbruikbaar, *"Instead of Handling Large Projects transactions e.g. the Project Builder transaction should be used."* | **bevestigd** letterlijk op exact de genoemde pagina's | idem |
| 15 | Simplification List §29.21 (p. 655–656): simulaties (CJV1–CJV7) per S/4HANA 2020 FPS2 / 2021 FPS1 naar **perpetual scope** | **bevestigd**; matrix-ID is **469** (het profiel noemde alleen ID 465 voor graphics) | idem |
| 16 | EPC ondersteunt **Oracle Primavera P6 EPPM 16.1 en 16.2**, laatste release **3.0 FP01** | **bevestigd** — de pagina heet letterlijk *"Oracle Primavera P6 EPPM 16.1 and 16.2 Support"* en zit in *"What's New in SAP Enterprise Project Connection 3.0 FP01"*; geen nieuwere EPC-release in de Help Portal aangetroffen | [SAP Help](https://help.sap.com/docs/SAP_ENTERPRISE_PROJECT_CONNECTION/4462b8e530aa4159bf9898f254c359aa/8c211fc1e7444e488e27c52ad41e94d2.html) |
| 17 | SAP SE: omzet **€36,8 mrd (2025)**, bedrijfsresultaat **€10,293 mrd**, **110.650 medewerkers** | **bevestigd** (FY2025) | [Wikipedia – SAP SE](https://en.wikipedia.org/wiki/SAP_SE) |
| 18 | PeerSpot: **4,4/5 (8,8/10)**, **5 reviews**, **100% willing to recommend**, **#11** in PPM, mindshare **3,7%** nu tegen **6,3%** een jaar eerder, top-sector **Construction 11%**, **48% large enterprise** | **bevestigd** — alle acht cijfers exact | [PeerSpot](https://www.peerspot.com/products/sap-portfolio-and-project-management-reviews) |
| 19 | SoftwareReviews: composite **7,7/10**, CX **8,1/10**, **19 reviews**, NEF **+96** (98% positief / 2% negatief), drie capabilities **90/100** tegen categoriegemiddelde **81** | **bevestigd**, inclusief het categoriegemiddelde van 81 (een eerste ophaalpoging gaf abusievelijk 87; bij gerichte hercontrole per capability bleek 81 correct) | [SoftwareReviews](https://www.softwarereviews.com/products/sap-portfolio-and-project-management) |

### Gecorrigeerd

| # | Oorspronkelijke bewering | Wat er werkelijk staat | Verwerkt in |
|---|---|---|---|
| C1 | **"PPM heeft geen total/free float-uitvoer en geen kritiek pad als eersteklas begrip."** | **Onjuist.** SAP levert voor PPM een instelbare *Alternative Calculation for Critical Path*: *"you can change the logic used to determine the critical path of a project… tasks with a float of less than or equal to zero are flagged as part of the critical path only if their delays jeopardize the adherence to the project dates."* Component **PPM-PRO**, beschikbaar vanaf **PPM 6.1 SP14 / 6.0 SP18**, ook uitgeleverd in de what's-new van S/4HANA 1709/1809/1909. PPM kent dus zowel *float* als *critical path*. Wat blijft: dagresolutie, geen P6-achtige float-rapportage, geen nivellering op netwerkniveau. | §2.1, §2.4, §8 — [bron](https://help.sap.com/docs/SAP_PORTFOLIO_AND_PROJECT_MANAGEMENT/47f3c81b7cc040f1aa74c2353651b6ac/898f5abb818d4a599646dd293018947a.html) |
| C2 | **"SAPUI5-Gantt: printen niet ondersteund."** | **Achterhaald.** *"Printing in Gantt Chart Based on SAPUI5 Technology… provides an option to print a Gantt chart visualization by generating a file in PDF format"* — **beschikbaar vanaf PPM 6.1 (SP14)**. Het profiel citeerde de SP13-what's-new zonder de latere SP14-toevoeging. *"The Print function is not supported"* geldt nog wél voor de **multi-project monitor**. | §2.4, §5.1 — [bron](https://help.sap.com/docs/SAP_PORTFOLIO_AND_PROJECT_MANAGEMENT/47f3c81b7cc040f1aa74c2353651b6ac/fa588556dcd048a5a87600940894ab3e.html) |
| C3 | **"De multi-project monitor is helemaal niet vervangen."** | **Onjuist.** Er is een *"Alternative Multi-Project Monitor Based on SAPUI5 Technology"*, en in de actuele documentatie is die de standaard: *"This SAPUI5-based multi-project monitor has replaced the former multi-project monitor based on Java Plug-In technology as the standard."* | §2.4, §5.1 — [bron](https://help.sap.com/docs/SAP_PORTFOLIO_AND_PROJECT_MANAGEMENT/213a68a4ef6e448e8e49d8f67d1a1607/48b8e1b3021910b5e10000000a421138.html) |
| C4 | **"Project Planning Board, netwerkgrafiek én hiërarchiegrafiek leven op geleende tijd (compatibility scope, vervaldatum)."** | **Te sterk.** §29.20 bevestigt de compatibility scope voor "Graphics in Project System" (ID 465), maar somt onder *"graphics that are only available in SAP GUI"* uitsluitend de **hiërarchiegrafiek** op en stelt over de andere twee juist: *"SAP GUI-based project planning board **will continue to be available**"* en *"SAP GUI-based network graphic **will continue to be available**"*, met de Fiori-apps als **aanvulling** ("In addition"). | §2.5, §5.3 — [SIMPL_OP2021.pdf p. 654–655](https://help.sap.com/doc/f2591a6901344c97a5e2029cc8f3703e/2021/en-US/SIMPL_OP2021.pdf) |
| C5 | Impliciet: de SAPUI5-Gantt is een forse functionele achteruitgang | **Genuanceerd.** SAP's actuele tekst (PPM 6.1 SP24, 14-05-2025) zegt: *"The SAPUI5-based Gantt chart offers largely the same functional scope and user experience as the former Gantt chart based on Java Plug-In technology"*, en de SAPUI5-variant *"has replaced the former Gantt chart… as the standard Gantt chart"*. De resterende beperkingen (geen navigatiefunctie, geen drag-and-drop-herordening in de hiërarchie, vier vaste zoomniveaus, Enter-bevestiging per veld) zijn wél bevestigd in diezelfde SP24-documentatie. | §2.4, §5.1, §8 — [bron](https://help.sap.com/docs/SAP_PORTFOLIO_AND_PROJECT_MANAGEMENT/213a68a4ef6e448e8e49d8f67d1a1607/b185db48420643a5acef3ef3a3565f4a.html) |
| C6 | "De Fiori-vervangers zijn read-/analyse-georiënteerd en bieden niet dezelfde bewerkingsdiepte" — gepresenteerd als bronfeit | **Herclassificeerd als inschatting van de analist.** In de SAP-documentatie is hierover geen uitspraak gevonden; §29.20 zegt alleen dát de apps bestaan en vanaf welke release. | §5.3 |

### Onzeker

| # | Bewering | Waarom onzeker |
|---|---|---|
| O1 | **S/4HANA Cloud Public Edition heeft géén netwerken/CPM-engine** | Al in het profiel als *inschatting van de analist* gemarkeerd, en dat blijft terecht. Gerichte zoekacties in de S/4HANA-Cloud-Public-Edition-documentatie leverden **geen expliciete bevestiging én geen weerlegging** op: er is geen SAP-pagina gevonden die zegt dat netwerken daar niet beschikbaar zijn. De redenering (alle PS-netwerkdocumentatie en CN\*/CJ\*-transacties staan uitsluitend onder on-premise/private) is plausibel maar blijft indirect. **Niet als feit presenteren.** |
| O2 | EPC-documentatie *"gepubliceerd 15 juni 2017"* | De Help-Portal-index geeft voor de EPC 3.0 FP01-what's-new-pagina's als datum **9 juni 2017**. Het verschil is triviaal en verandert de conclusie (bevroren sinds 2017) niet, maar de exacte dag is niet hard te maken. |
| O3 | Vendr *"gemiddelde korting 24,16%"* | Vendr toont afgerond **"Buyers save 24% on average"**. De twee decimalen zijn niet terug te vinden op de publieke pagina. |
| O4 | Schattingen over projectomvang (comfortabel enkele honderden; 1.000–5.000 haalbaar; >10.000 onwerkbaar) | Expliciet als **inschatting van de analist** gemarkeerd en niet verifieerbaar: SAP's eigen richtlijn staat in de niet-publieke **Note 206264**. Blijft als schatting staan. |
| O5 | *"Enkele tienduizenden organisaties met een actief ingerichte PS-module"* | Expliciet als schatting gemarkeerd; SAP publiceert geen module-omzet of installed base. Niet verifieerbaar, niet weerlegbaar. |
| O6 | Vervaldatum van de compatibility scope (Note 2269324) | Note vereist SAP-login; **niet geverifieerd** — zoals het profiel zelf al aangeeft. |
| O7 | Valuta-omrekening £→€/$ (§3.2) | Expliciet als indicatieve schatting van de analist gemarkeerd; wisselkoersafhankelijk. |

### Wat níet weerlegd kon worden

De **kern van het profiel houdt stand**: de prijsstelling is tot op de pence exact reproduceerbaar uit de primaire bronnen; de CPM-engine van SAP PS is met letterlijke SAP-citaten bevestigd (voor-/achterwaartse pass, total en free float, kritiek pad als minimale-float-pad, minimum-only lags); PPM's dagresolutie is bevestigd in de nieuwste release; de geschrapte interfaces en het obsolete *Handling Large Projects* staan letterlijk op de geciteerde pagina's van de Simplification List; en de marktcijfers van PeerSpot, SoftwareReviews en Vendr kloppen zonder uitzondering. De correcties raken uitsluitend **PPM's planningsfunctionaliteit (C1)** en de **hardheid van de UI-/compatibility-scope-kritiek (C2–C6)** — die was op onderdelen gebaseerd op verouderde release-notities in plaats van de actuele documentatie.

*Fact-check uitgevoerd met volledige uitlezing van de drie G-Cloud-PDF's (pypdf), de Simplification List op paginaniveau, de SAP Help content-API en directe fetches van Vendr, PeerSpot, SoftwareReviews, Wikipedia en de G-Cloud-listing. WebSearch was in deze sessie niet beschikbaar (quotum uitgeput); alle verificatie liep via directe bron-URL's.*
