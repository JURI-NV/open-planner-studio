...
Tabbladen (`RibbonTab`): `file`, `start`, `planning`, `resources`, `relations`, `beeld`, `instellingen`, `table`, `ifc`, `report`, `ai` — die laatste verschijnt alleen als `ui.aiMode` aan staat.
...
- [docs/superpowers/](docs/superpowers/) — actieve ontwerp- en implementatiedocs (`specs/` + `plans/`: auto-update, snap-packaging, multi-worktree-dev-isolatie, dev-server dual-guard, UI-overhaul, planning-correctheid-testplan, B1.1-bibliotheekmodel, MCP-bridge fase 1, "je bent net geüpdatet"-dialoog, werkdagen-as en verticale drag, B1b-bezettingsoverzicht, mijlpaal-/samenvattingstaakrelaties + spookrelaties, taskType-overerving van de bovenliggende taak).
