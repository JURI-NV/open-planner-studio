...
truthy('04 export heeft een bestand geschreven', typeof captured === 'string' && captured.length > 0);
const xml = captured ?? '';
...
eq('08 import: precies één baseline terug', back.baselines.length, 1);
truthy('09 import: baseline is actief', !!back.activeBaselineId && back.activeBaselineId === back.baselines[0]?.id);

const backBaseline = back.baselines[0];
