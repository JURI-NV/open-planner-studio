...
export interface RecoveryNames {
  ...
  legacy: string;
  /** Bestandsnaam van de snapshot van één document. */
...
export interface RecoveryManifest {
   ...
   * `heartbeatAt`. Een v1-manifest MOET leesbaar blijven: het staat op de schijf van iedereen die
   * de vorige versie draaide, en dat weigeren betekent dataverlies bij de eerste start na de
   * update. Beide velden zijn daarom optioneel getypeerd en het versienummer wordt nergens als
   * poort gebruikt — alleen als informatie.
   */
  version: number;
