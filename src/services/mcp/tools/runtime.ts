 ...
 *   - `paused`/`readOnly` — de twee veiligheidsvlaggen uit de ui-state. `McpContext.paused/readOnly`
 *     zijn zelf een snapshot hiervan (buildMcpContext); omdat de bindende signatuur `buildEnvelope()`
 *     géén ctx neemt, lezen we hier live — dezelfde waarde op call-moment, en de envelop blijft óók
 *     zonder ctx gezaghebbend.
 ...
 *   4. drift: `expectedDocId` gezet én ≠ actief doc ⇒ `DOC_DRIFT` ("was X, nu Y — bevestig met
 *      switch_document"). Is `expectedDocId` nog null, dan bindt deze eerste mutatie het anker aan
 *      het actieve doc.
 *   5. `await ctx.ensureBackup(docId, kind)` — een pad wordt een envelop-veld; een throw/reject ⇒
 *      `BACKUP_FAILED` VÓÓR enige mutatie (fail-safe, geen rollback nodig). De backup-await staat
 *      bewust ná de drift-check-binding maar de transactie draait ná de await; een eventuele
 *      tabwissel tijdens de await zou een nieuwe call zijn, niet deze synchrone transactie.
 *   6. `runInMcpTransaction(fn…)` — bij succes komen `outcome.data` + `itemRejections` in de Ok-
 *      respons; een transactie-fout wordt via `mapTransactionError` een `CYCLE`/`VALIDATION`-fout.
 */
export async function runMutateTool(
  ...
  if (ctx.expectedDocId === null) {
    ctx.expectedDocId = activeId; // eerste mutatie bindt het anker aan het actieve document
  }

  // (5) AI-backup: async, VÓÓR de synchrone transactie (WP0-invariant b). Mislukt hij, dan weigeren
  //     we vóór er iets gemuteerd is — geen rollback nodig.
  let backupPath: string | null = null;
  try {
    backupPath = await ctx.ensureBackup(activeId, kind);
  } catch (e) {
