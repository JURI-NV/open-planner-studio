...
- The docked resource panel next to the Gantt, as a second panel below Properties.
...
Open the resource panel via the **Manage** ribbon group on the **Resources** tab: the **Resources** button opens the full panel (a separate full-panel view, like Table or Relations), **New resource** opens the panel with a draft row in which you type the name straight away (just like the **+ New resource** button in the panel itself). In the panel you edit, per resource: **Name**, **Type**, **Max units** (capacity per work day — 1 = one person/item full time, 2 = two units at once), **Calendar**, **Rate/hour**, **Unit** (material only) and **Crew** (which crew this resource belongs to). At the bottom, the **Total** column adds up each resource's cost (loaded units × hours/day × rate), recalculated on every F5.

The draft row sits at the bottom of the table, exactly where the new resource will end up, and it is *fully* editable straight away: type, max units, calendar, rate, unit and crew are all ordinary controls. Fill them in whatever order suits you — feel free to pick the type first and type the name afterwards. **Tab** walks you through the row without saving anything yet.

The row exists for one reason: a resource is only created once there is a **name**, so that pressing the button without entering anything doesn't leave an empty row behind. Clicking away or pressing **Escape** without a name leaves nothing behind — no resource, no step in the undo history, and your project isn't marked as changed; not even if you changed a dropdown along the way. With a name in place, the row is committed the moment you leave it (click elsewhere, or Tab past the last column) or press **Enter** — in one go, with everything you filled in, and as a single step that one undo takes back. **Enter** also opens a fresh draft row right below it, so you can type a whole list in one go.

The existing rows are keyboard-navigable too, in both views (Library and Project): **↑** and **↓** move the cursor to the same column in the row above or below, and **Enter** (down) or **Shift+Enter** (up) does the same. On the very last row, **Enter** opens a new draft row there — the same continuous entry as in the task table. In the dropdowns (**Type**, **Calendar**, **Crew**) and in the **Max units** spinner the arrow keys deliberately keep their own meaning (picking an option, stepping a value); use **Enter** there to move on to the next row.
...
You split the height by dragging the **boundary between the two panels**; that split is remembered. When only one panel is on, it always gets the full height. With both off there is no side column at all and the Gantt gets the full width.

To get the column out of the way *without* losing your panel choice, use the button at the top right of the upper header bar. You're left with a narrow strip; one click on it brings the column back exactly as you left it.

The same three buttons — **Resources**, **Resource dock** and **Histogram** — also sit on the **View** tab in the **Panels** ribbon group, next to **Properties**. Same name, same icon, same behaviour: wherever you click them, they do exactly the same thing. The difference is between the two resource buttons themselves: **Resources** opens the full panel over the workspace, **Resource dock** pins the compact list as a second panel in the side column.
