...
The draft row sits at the bottom of the table, exactly where the new resource will end up. It exists for one reason: a resource is only created once there is a name, so that pressing the button without typing anything doesn't leave an empty row behind. Type nothing and click away (or press **Escape**) and nothing is left behind — no resource, no step in the undo history, and your project isn't marked as changed. As soon as you type a name and want to move on with **Tab** or **Enter**, the resource really exists and *all* of its fields are editable as usual: **Tab** puts you straight into the **Type** column of that same row, **Enter** commits the resource and opens a fresh draft row right below it, so you can type a whole list in one go.
...
The same two toggles also sit on the **View** tab in the **Panels** ribbon group, next to **Properties**. There they are named after the panel they show: **Resources** (the same button as **Dock** above) and **Histogram**. Wherever you click them, they do exactly the same thing.
